...
- **导入 Token `token`**：要和后端「货源配置」（`sources` 配置分类下的 `import_token`）一致，ingest 接口用 `X-Import-Token` 头做鉴权，不一致会 401。
  - 目前前端「货源配置」页（`/settings/sources`）还没有暴露 `import_token` 和 `import_1688_*_path` 这几个字段的表单（只做了 `ali1688_image_search_url`/`ali1688_keyword_search_url`/`ali1688_method`/`ali1688_extra_params`/`ali1688_extra_headers`/`ali1688_offer_list_path`），需要用管理员账号登录拿到 JWT 后，直接调 `PUT /settings/sources` 写入，例如：
    ```bash
    ...
    把同一个 token 值填进扩展 options 的「导入 Token」。未来后端「货源配置」页可能会补上 `import_token`/`import_1688_*_path` 的填写入口，届时可不用走 curl。
