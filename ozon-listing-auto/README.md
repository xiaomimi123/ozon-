...
| `OZON_SELLER_PROVIDER` | Ozon 跟卖/自建挂靠：`mock`（默认，无需真实凭据）或 `real`（真实调用 Ozon Seller API，需店铺真实凭据）。**同时影响异步/worker 路径**（`run_publish`、`run_publish_tick` 及其 cron）**和 `sync=true` 的同步路径**（`/listing/publish`、`/publish/tick`）——两者均经 `resolve_seller()` 按 `system` 配置的 `ozon_seller_provider`（本环境变量为兜底默认值）解析 provider。`real` 模式下是否真正提交由 DB 配置项 `system.ozon_publish_dry_run`（非环境变量，系统设置页「试运行(dry-run)」开关，默认开）控制：开启时只构造真实 Ozon 请求但不提交，见 M4/M6 章节与下方「真实 Ozon 上架说明」 | `mock` |
...
| `IMAGE_PROVIDER` | 声明保留位，当前未被读取——`gen`（AI 生图）op 实际走 `/settings/imagegen`（DB 配置，见「外部生图说明」）而非此环境变量选 provider。**`rmbg`/`whitebg`/`watermark`/`crop_norm` 四个本地类 op 恒走 Pillow `LocalProvider`，不受此变量影响**；改图默认流水线（`POST /images/process`）只跑 `whitebg`+`crop_norm`，不含 `rmbg`/`gen`。`rmbg` 去背景需 `rembg`（属 `[ml]` 组，随 `INSTALL_ML=true` 一起装），未安装时自动降级为白底（`meta.degraded=true`），不会报错中断 | `mock` |
...
├── server/               # FastAPI 后端
│   ├── app/
│   │   ├── api/          # 路由（auth/users/tasks/collect/products/settings/ws/accounts/match/candidates/
...
└── web/                  # React + Vite 前端
    ├── src/
    │   ├── api/
    │   ├── pages/         # 登录（品牌化 2:1 分栏+移动端单列）、任务中心、商品列表、审核台（ReviewBoard）、
    │   │                  #   上架审核（ListingReview，含自建类目/属性/图确认）、店铺管理（Shops）、上架监控
    │   │                  #   （PublishMonitor，节奏配置+队列监控+实时 WS）、图片工作室（ImageStudio，
    │   │                  #   /image-studio）、AI 生图配置（settings/ImagegenSettings，/settings/imagegen）、
    │   │                  #   员工管理（settings/StaffSettings，/staff，admin 可见）等页面
    │   └── store/
...
- **Ozon 写入抽象**（`app/services/ozon_seller/`，`OzonSellerProvider` 协议 `create_follow_offer()`）：
  ...
  - `RealOzonSeller`（`get_ozon_seller("real")`）：以店铺 `Client-Id`/`Api-Key` 调**真实 Ozon Seller API**（`api-seller.ozon.ru`）——跟卖 `create_follow_offer` 走 `POST /v1/product/import-by-sku`、自建 `create_product` 走 `POST /v3/product/import`、审核轮询 `get_product_status` 走 `POST /v1/product/import/info`，**三个端点已对齐真实 Ozon Seller API**（子项目 E，详见下方「真实 Ozon 上架说明」）；自建草稿现已补齐 `type_id`/尺寸/重量/属性字典值 id（M6 追加，见下方「M6 已实现功能」补充说明与「真实 Ozon 上架说明」）。挂靠 seller 由 `resolve_seller()` 按 `system` 配置的 **`ozon_seller_provider`**（`mock`/`real`，环境变量 `OZON_SELLER_PROVIDER` 为兜底默认值 `mock`）解析，**同时覆盖异步/worker 路径**（`POST /listing/publish?sync=false` 的 `run_publish`、`POST /publish/tick?sync=false` 入队后由 cron/`run_publish_tick` 执行）**与 `sync=true` 的同步路径**（两个接口皆是）。`real` 模式下默认还开着 `system.ozon_publish_dry_run`（试运行，只构造真实请求不提交），关闭后才会真实提交，本地/演示/测试建议保持开启避免误触真实 Ozon 写入。要在生产环境真实挂靠，把系统设置切到 `real` 并关闭试运行、配好店铺真实凭据即可，无需再改代码；详见 [`docs/M4-定价上架说明.md`](docs/M4-定价上架说明.md) 第 5 节与 [`docs/M5-节奏调度说明.md`](docs/M5-节奏调度说明.md)。
...
- **周期调度**（`app/workers/arq_worker.py`）：`run_publish_tick`（ARQ cron，每分钟触发一次）扫描所有存在到期 `scheduled` 或 `pending_review` 草稿的任务并逐个 `tick_publish`；也可手动触发 `POST /publish/tick?task_id=<id>&sync=true|false`（publisher/admin）——`sync=true` 请求内同步跑一次，`sync=false`（默认）经 ARQ 入队立即触发一次 `run_publish_tick`；两条路径均按 `system.ozon_seller_provider`/`ozon_publish_dry_run` 选 seller 与是否试运行。
...
- **`OzonSellerProvider` 新增 `get_product_status()`**（M5 起协议方法，`app/services/ozon_seller/base.py`）：`MockOzonSeller` 默认返回 `approved`，可在构造时注入 `pending_ids` 集合使指定 `ozon_product_id` 返回 `pending`，供测试/本地演示模拟「审核中」；`RealOzonSeller` 已真实请求 `POST /v1/product/import/info`（`task_id` 即 create 返回的导入任务 id），`imported`→`approved`、`failed`→`rejected`，其余状态（含请求异常）→`pending`。
...
- **改图流水线**（`app/services/imagegen/` + `app/workers/imager.py::run_image_process_core`，`product_images` 表）：对任务下已采用候选的源图逐张跑一组 op，每 (候选, op) 产出一行 `product_images` 记录（含 `status`：`pending`/`processing`/`done`/`failed`、`result_url`、`provider`），单图下载/处理失败仅标记该行 `failed` 并记录 `error`，不影响同批其余图片。默认流水线 `ops=["whitebg", "crop_norm"]`（白底 + 裁剪归一）。
  - **Provider 分派**（`app/services/imagegen/factory.py::process_op`）：本地类 op（`rmbg` 去背景 / `whitebg` 白底 / `watermark` 去水印 / `crop_norm` 裁剪归一）恒走 `LocalProvider`（Pillow 真实处理，不受 `IMAGE_PROVIDER` 影响）；`gen`（AI 生图，**默认 `ops` 列表未启用，属 opt-in**）才按 `get_configured_gen_provider(session, static_dir=...)` 从 `/settings/imagegen` 配置构造 provider 分派——`openai_compat`（真实调用标准 `images/generations` 接口）/`http`（可配请求模板+响应取图点路径，接任意 HTTP 生图服务）已真实可用，详见下方「外部生图说明」；未配置或无 key 时回退 `mock`（占位假 URL）。
...
- **自建建品**（`app/services/ozon_seller/`，`create_product()`，`_call_seller()` 按 `draft.mode` 分派）：`MockOzonSeller.create_product` 确定性返回成功、`ozon_product_id="OZC-{offer_id}"`，供本地/CI/演示全链路验证；`RealOzonSeller.create_product` 真实调用 `POST /v3/product/import`，草稿携带的 `type_id`/尺寸(`depth`/`width`/`height`)/`weight`/单位与属性字典值 id（见下一条「自建补充信息」）随请求一并提交。上架路径与跟卖分支共用 M5 节奏调度（`plan_schedule`/`tick_publish`），两条路径均按 `system.ozon_seller_provider`/`ozon_publish_dry_run` 控制真发/试运行，详见下方「真实 Ozon 上架说明」。
- **自建补充信息（`type_id`/尺寸/重量/属性字典值）**（迁移 `0007_auto_listing_create_fields`）：`listing_drafts` 新增 `type_id`、`depth`/`width`/`height`、`weight`、`dimension_unit`（默认 `mm`）、`weight_unit`（默认 `g`）字段。自建草稿确认闸门（`confirm_draft`）在原有「类目/图片」校验基础上追加：`type_id` 或尺寸/重量任一缺失时 `POST /listing/{id}/confirm` 拒绝放行并提示「自建缺类型或尺寸/重量, 请先补充」；新增 `POST /listing/{draft_id}/confirm-create-fields` 写回这些字段（含可选 `attributes` 增量合并）。配套新增 Ozon 卖家端类目查询 API `GET /ozon-catalog/types|attributes|attribute-values`（operator 及以上，`app/api/ozon_catalog.py`）：有真实 Ozon 店铺且 `system.ozon_seller_provider=real` 时走真实类目/类型/属性接口，否则回退样例数据；属性值经此接口取到的字典值 id 会作为 `{"dictionary_value_id": ...}` 写入 `attributes`，供 `create_product` 转换为 Ozon 要求的 `[{"complex_id":0,"id":<attr_id>,"values":[...]}]` 结构。前端 **ListingReview**（`/listing`）上架审核页对 `mode="create"` 的草稿新增「补充信息」表单（类型/尺寸/重量/必填属性选择），确认前需先在此填齐。**已知限制**：`attributes` 字典的键必须是数字（Ozon attribute id）；非数字键会在挂靠时使该草稿标记为 `failed`（写明错误原因，不再静默丢弃）。
- **AI 生图配置中心**（`GET/PUT /settings/imagegen`，admin，`app/api/imagegen.py`）：`provider`（`mock`/`local`/`openai_compat`/`http`）、`img_base_url`、`img_api_key`（`FERNET_KEY` 加密存储，`GET` 脱敏返回 `"***"`，`PUT` 时留空不覆盖已存密钥）、`img_model`、`fallback`（降级顺序）、`img_request_template`/`img_response_path`（仅 `http` provider 用的请求体模板与响应取图点路径）。前端 **AI 生图配置页**（`/settings/imagegen`，`ImagegenSettings.tsx`）。`process_op`/`run_image_process_core` 已接入此配置真实构造 gen provider，详见下方「外部生图说明」。
...
- **品牌**：系统名「智选万物」，登录页与侧边栏统一展示 logo/名称（`web/src/brand.ts`）。
- **角色**：`管理员`（admin）/ `操作员`（operator）/ `审核员`（reviewer）/ `发布员`（publisher）四种角色，登录后 JWT 携带角色信息，侧边栏菜单按角色隐藏管理类入口（前端隐藏仅为体验优化，实际权限以后端各 API 的角色校验为准）。
- **用户管理 API**（`/users`，admin，`app/api/users.py`）：`GET /users`（列出全部员工，`UserOut` 不含密码哈希）、`POST /users`（新建员工，`username`/`password`/`role`）、`PUT /users/{id}`（改角色 `role` 和/或启停 `is_active`）、`POST /users/{id}/password`（重置密码）、`DELETE /users/{id}`（删除）；均内置**防锁死**校验（不能停用/降权/删除系统里最后一个可用管理员），触发时返回 `400` + 中文提示（如「至少保留一个可用管理员」），前端 `message.error` 直接透出该提示。
- **员工管理页**（`/staff`，admin 可见，`web/src/pages/settings/StaffSettings.tsx`）：员工列表（用户名/角色标签/启用停用状态/创建时间）+ 新增员工表单 + 行内操作（改角色下拉、启用/停用按钮、重置密码弹窗、删除二次确认），任意操作后自动刷新列表。
- 启动种子（`ADMIN_USER`/`ADMIN_PASSWORD`）创建的首个管理员默认角色为 `admin`，用于登录后再通过 `/staff` 页面创建其余角色的员工账号。

## 测试

后端（190 个用例，另有 9 个标记为 `live` 的真实网络冒烟测试默认跳过，覆盖 M1 采集 + M2 货源匹配 + M3 评分审核 + M4 定价上架 + M5 节奏调度 + M6 自建改图/类目映射 + M7 CORS/登录限流 + 员工/用户管理 + 真实爬虫接入 + 真实 LLM 全链路 + 真实类目树 + 真实 CLIP 向量 + 外部生图 + 真实 Ozon Seller 上架）：
...
4. **Seller real/mock 切换**（挂靠上架，非采集）：`/settings/system`（admin）「上品模式」切 `ozon_seller_provider` 为 `mock`/`real`，同时影响 `sync=true` 同步路径与 `run_publish`/`run_publish_tick` 异步路径；`real` 下还有「试运行(dry-run)」开关 `ozon_publish_dry_run`（默认开，只构造真实请求不提交）。
...
M6 的 AI 生图配置中心（`/settings/imagegen`）已接入两个真实 provider，供自建分支的 `gen`（AI 营销图）op 使用：

- **`openai_compat`**（`OpenAICompatImageProvider`）：真实调用标准 OpenAI 图像接口 `{img_base_url}/images/generations`（Bearer 鉴权），默认适配千问万相等 OpenAI 兼容端点；响应按 `data[0].url` 下载或 `data[0].b64_json` 解码后落盘到 `server/static/images/`。填好 `img_base_url`/`img_api_key`/`img_model` 即可。
- **`http`**（`HttpImageProvider`）：通用 HTTP 适配器，额外需配 `img_request_template`（请求体 JSON 模板，用 `{prompt}`/`{model}` 占位）与 `img_response_path`（响应取图 URL 的点路径，如 `data.0.url`），用于接入非 OpenAI 格式的生图服务（如 GRSAI/云舞AI）；两项均在 `/settings/imagegen` 页面填写，非密钥、明文存储。
- 未配置或缺 `img_api_key`/`img_base_url` 时，`get_configured_gen_provider`（`app/services/imagegen/config.py`）自动回退 `mock`（占位假 URL），不影响其余改图 op。
- **gen 是 opt-in**：改图流水线默认 `ops=["whitebg", "crop_norm"]`，不含 `gen`；`gen` op 需在调用 `run_image_process_core(..., ops=[...,"gen"])` 时显式传入（当前 `POST /images/process` API 未暴露 `ops` 参数），真实生图链路以下方 `@live` 冒烟测试直接验证。
- 只有异步路径（`POST /images/process?sync=false` 经 ARQ 的 `run_image_process`）会按此配置构造真实 gen provider；`sync=true` 同步路径仍固定用 `mock`（该区分目前只保留在改图/生图这一处，上品 `OZON_SELLER_PROVIDER` 已在 M6 之后改为 sync/async 两条路径统一走配置，见「真实 Ozon 上架说明」）。

`@live` 真实生图冒烟测试（默认跳过，不影响日常回归）：

```bash
cd server
IMG_API_KEY=sk-... IMG_BASE_URL=https://... IMG_MODEL=wanx-v1 .venv/bin/python -m pytest tests/test_live_imagegen.py -m live -v
```

未设 `IMG_API_KEY` 时用例自动 `skip`；`IMG_BASE_URL`/`IMG_MODEL` 未设时默认空 base url / `wanx-v1`；断言一次真实 `OpenAICompatImageProvider.process(op="gen")` 调用返回以 `/static/images/` 开头的图片 URL。

## 真实 Ozon 上架说明

`app/services/ozon_seller/real.py::RealOzonSeller` 已对齐**真实 Ozon Seller API**（`api-seller.ozon.ru`，`Client-Id` + `Api-Key` 请求头鉴权）：

- **跟卖 `create_follow_offer`** → `POST /v1/product/import-by-sku`：按目标 Ozon SKU 克隆卡片建 offer；响应 `result.unmatched_sku_list` 非空（SKU 不可跟卖/未匹配）视为失败；成功则 `ozon_product_id` 取返回的导入 `task_id`（状态 `pending_review`，需继续轮询）。
- **自建 `create_product`** → `POST /v3/product/import`：按草稿的标题/类目/属性/图片/价格/`type_id`/尺寸(`depth`/`width`/`height`)/`weight`/单位构造商品创建请求，同样返回 `task_id`。这些字段（迁移 `0007`）与配套的 `GET /ozon-catalog/types|attributes|attribute-values` 查询 API、上架审核页「补充信息」表单、`POST /listing/{id}/confirm-create-fields` 回写端点，以及确认闸门对 `type_id`/尺寸/重量的强制校验，已在 M6 之后补齐（详见上方「自建补充信息」小节）。**仍存的已知限制（诚实标注）**：`attributes` 字典的键必须是数字型 Ozon attribute id——非数字键会在 `int(k)` 转换时抛错，被上架流程捕获后把该草稿标记为 `failed`（记录错误原因，不再静默丢弃），需人工核对属性 id 后重试。
- **审核轮询 `get_product_status`** → `POST /v1/product/import/info`（`{"task_id": ...}`）：`imported` → `approved`、`failed` → `rejected`，其余状态（含 `pending`、请求异常）一律映射为 `pending`，供 M5 节奏调度的等审核门轮询。
- **价格**：定价引擎算出的浮点售价（如 `2300.0`）会转成 Ozon 期望的干净数值字符串（`"2300"`），非整数保留原值。
- **属性**：草稿的 `{attr_id: value}` 字典会转换为 Ozon 要求的 `[{"complex_id": 0, "id": <attr_id>, "values": [...]}]` 结构；值为 `{"dictionary_value_id": <int>}`（来自 `/ozon-catalog/attribute-values` 选值）时走属性字典值 id，否则按标量/`{"value": ...}` 走自由文本 `value`。
- **库存未包含在本轮**：Ozon 库存需单独调用 `POST /v2/products/stocks` 设置，当前实现的价格在 `import`/`import-by-sku` 请求体内一并提交，库存对接留作后续。

**如何切到真实调用**：

1. 在「店铺管理」（`/shops`）填真实/沙箱 Ozon 店铺的 `Client-Id`/`Api-Key`（Fernet 加密存储）。
2. 在「系统设置」（`/settings/system`，admin）把「上品模式」切到 **真实（调用 Ozon）**（对应 `system.ozon_seller_provider=real`，环境变量 `OZON_SELLER_PROVIDER` 仅为兜底默认值），此项同时对 `sync=true` 同步路径与异步/worker 路径（`run_publish`/`run_publish_tick` 及其 cron）生效；「试运行(dry-run)」开关（`system.ozon_publish_dry_run`，默认开）建议先保持开启——`real` + 开着试运行会构造真实 Ozon 请求但不提交，验证无误后再关闭试运行才会真正提交。
3. 跟卖走通需要一个**真实存在、允许跟卖的目标 Ozon SKU**；自建除上方「仍存的已知限制」（属性字典键须为数字 id）外，还需先在上架审核页对该草稿补齐「补充信息」（类型/尺寸/重量/必填属性），否则确认闸门会拒绝放行。

**`@live` 沙箱冒烟测试**（默认跳过，不影响日常回归）：
...
- **M6**：自建改图与类目映射（已完成）——改图流水线（provider 抽象 mock/local(Pillow)/openai_compat/http，rmbg 降级白底）、类目属性映射三级兜底（记忆表/LLM/兜底）+ 记忆表复用、自建草稿生成、类目+图确认闸门、自建建品（`RealOzonSeller.create_product` 真实对接 `/v3/product/import`；草稿已补齐 `type_id`/尺寸/重量/属性字典值 id，确认闸门强制校验，`GET /ozon-catalog/*` 查询 + 上架审核页「补充信息」表单补齐，见「真实 Ozon 上架说明」）、`/images`、`/categories`、`/settings/imagegen` API + 前端 ImageStudio/ImagegenSettings；同步/异步上品路径均按系统设置的 `ozon_seller_provider`/`ozon_publish_dry_run` 生效（见 M4 小节）
