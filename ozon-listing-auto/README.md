...
- **登录失败限流**（`app/core/auth.py` + `LoginThrottle`，`LOGIN_MAX_ATTEMPTS`/`LOGIN_WINDOW_SEC`/`LOGIN_LOCKOUT_SEC`）：按 `用户名|IP` 维度滑动窗口计数，窗口内失败达到上限则锁定，`/auth/login` 锁定期间返回 `429` + `Retry-After`，登录成功或锁定到期后自动重置计数。
...
后端（133 个用例，默认跳过标记为 `live` 的真实网络冒烟测试，覆盖 M1 采集 + M2 货源匹配 + M3 评分审核 + M4 定价上架 + M5 节奏调度 + M6 自建改图/类目映射 + M7 CORS/登录限流全链路）：
