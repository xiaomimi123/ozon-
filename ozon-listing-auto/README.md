...
- **Ozon 写入抽象**（`app/services/ozon_seller/`，`OzonSellerProvider` 协议 `create_follow_offer()`）：
  ...
  - `RealOzonSeller`（`get_ozon_seller("real")`）：以店铺 `Client-Id`/`Api-Key` 调**真实 Ozon Seller API**（`api-seller.ozon.ru`）——跟卖 `create_follow_offer` 走 `POST /v1/product/import-by-sku`、自建 `create_product` 走 `POST /v3/product/import`、审核轮询 `get_product_status` 走 `POST /v1/product/import/info`，**三个端点已对齐真实 Ozon Seller API**（子项目 E，详见下方「真实 Ozon 上架说明」），自建仍有已知数据缺口（`type_id`/尺寸/属性字典 id）。M5 起挂靠 seller 由环境变量 **`OZON_SELLER_PROVIDER`**（`mock`/`real`，默认 `mock`）控制，但**只影响异步/worker 路径**（`POST /listing/publish?sync=false` 的 `run_publish`、`POST /publish/tick?sync=false` 入队后由 cron/`run_publish_tick` 执行）；`sync=true` 的同步路径（两个接口皆是）**仍固定使用 `mock`**，便于本地/演示/测试快速看到结果而不误触真实 Ozon 写入。要在生产环境真实挂靠，设置 `OZON_SELLER_PROVIDER=real` 并配好店铺真实凭据、始终走 `sync=false` 异步路径即可，无需再改代码；详见 [`docs/M4-定价上架说明.md`](docs/M4-定价上架说明.md) 第 5 节与 [`docs/M5-节奏调度说明.md`](docs/M5-节奏调度说明.md)。
...
- **自建建品（mock-first）**（`app/services/ozon_seller/`，`create_product()`，`_call_seller()` 按 `draft.mode` 分派）：`MockOzonSeller.create_product` 确定性返回成功、`ozon_product_id="OZC-{offer_id}"`，供本地/CI/演示全链路验证；`RealOzonSeller.create_product` 已真实调用 `POST /v3/product/import`，**但真实自建还需 `type_id`（类目下商品类型）+ 尺寸/重量 + 属性字典值 id，现有 `listing_drafts` 不带这些字段**，真实调用大概率会被 Ozon 审核拒绝，需后续扩草稿字段/人工补齐（详见下方「真实 Ozon 上架说明」）。上架路径与跟卖分支共用 M5 节奏调度（`plan_schedule`/`tick_publish`），由 `OZON_SELLER_PROVIDER` 环境变量控制异步路径用 mock 还是 real（`sync=true` 同步路径固定 mock，语义同 M4/M5）。
...
- 只有异步路径（`POST /images/process?sync=false` 经 ARQ 的 `run_image_process`）会按此配置构造真实 gen provider；`sync=true` 同步路径仍固定用 `mock`，语义同 M4/M5 的 `OZON_SELLER_PROVIDER` sync/async 区分。
...
- **自建 `create_product`** → `POST /v3/product/import`：按草稿的标题/类目/属性/图片/价格构造商品创建请求，同样返回 `task_id`。**已知数据缺口（诚实标注）**：真实自建审核除本字段外还需要 `type_id`（该类目下的具体商品类型）+ 尺寸/重量 + 属性字典值 id，而当前 `listing_drafts` 不携带这些字段，真实调用大概率会被 Ozon 审核拒绝——需后续扩展草稿字段采集流程（类目映射时一并拿到 `type_id`/属性字典，改图/自建表单补尺寸重量）或人工补齐后再上架，**不要假设自建在真实环境端到端可用**。
...
- **属性**：草稿的 `{attr_id: value}` 字典会转换为 Ozon 要求的 `[{"complex_id": 0, "id": <attr_id>, "values": [{"value": <value>}]}]` 结构。
...
2. 设置环境变量 `OZON_SELLER_PROVIDER=real`（或 M7 起在 `/settings/system` 把 `ozon_seller_provider` 切到 `real`），且**只对异步/worker 路径生效**（`POST /listing/publish?sync=false`、`POST /publish/tick?sync=false` 及其 cron）；`sync=true` 的同步演示路径固定用 `mock`，不受此项影响。
3. 跟卖走通需要一个**真实存在、允许跟卖的目标 Ozon SKU**；自建受限于上述数据缺口，暂不保证审核通过。
...
- **M6**：自建改图与类目映射（已完成）——改图流水线（provider 抽象 mock/local(Pillow)/openai_compat/http，rmbg 降级白底）、类目属性映射三级兜底（记忆表/LLM/兜底）+ 记忆表复用、自建草稿生成、类目+图确认闸门、自建建品（mock-first，`RealOzonSeller.create_product` 已真实对接 `/v3/product/import`，但缺 `type_id`/尺寸/属性字典 id，见「真实 Ozon 上架说明」）、`/images`、`/categories`、`/settings/imagegen` API + 前端 ImageStudio/ImagegenSettings
