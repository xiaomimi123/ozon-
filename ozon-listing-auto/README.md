...
后端（180 个用例，另有 9 个标记为 `live` 的真实网络冒烟测试默认跳过，覆盖 M1 采集 + M2 货源匹配 + M3 评分审核 + M4 定价上架 + M5 节奏调度 + M6 自建改图/类目映射 + M7 CORS/登录限流 + 真实爬虫接入 + 真实 LLM 全链路 + 真实类目树 + 真实 CLIP 向量 + 外部生图 + 真实 Ozon Seller 上架）：
